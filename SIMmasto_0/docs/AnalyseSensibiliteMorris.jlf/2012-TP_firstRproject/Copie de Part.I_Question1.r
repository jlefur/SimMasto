...
ssr = random_ss_regles_152params_Y1
amp = random_MPA_152params_Y1
tac = random_TAC_152params_Y1
...
for(i in 1:6){ # les 6 variables de sortie
  y = 152+i
  jpeg(filename = paste("F:/Formation/Atelier SA/Graphiques/PIQ1_",OutputsCol$output[i],"_histo.jpg",sep=""), width = 900)
    par(mfrow = c(1,3))
...
### Relation Y ~ xi
library(lattice)
for(ms in 1:3){                       # pour chaque mesure de gestion
  ...
  for(i in 1:6){                      # pour chaque Y
    ...
    for(j in c(0,42,84)){             # pour les differents X  (tout ne tient pas dans un graph, on traite les X, 42 x 42)
      jpeg(filename=paste("F:/Formation/Atelier SA/Graphiques/PIQ12/PIQ1_",OutputsCol$output[i],"_",namesMgtStr[ms],"_V",j+1,"-",j+42,".jpg",sep=""), width = 900, height = 900)
    ...
    j = 126
    jpeg(filename=paste("F:/Formation/Atelier SA/Graphiques/PIQ12/PIQ1_",OutputsCol$output[i],"_",namesMgtStr[ms],"_V",j+1,"-",j+26,".jpg",sep=""), width = 900, height = 900)
      plot(xyplot(planexp[,y] ~ planexp[,j+1]+planexp[,j+2]+planexp[,j+3]+planexp[,j+4]+planexp[,j+5]+planexp[,j+6]+planexp[,j+7]+planexp[,j+8]
      +planexp[,j+9]+planexp[,j+10]+planexp[,j+11]+planexp[,j+12]+planexp[,j+13]+planexp[,j+14]+planexp[,j+15]+planexp[,j+16]+planexp[,j+17]+planexp[,j+18]
