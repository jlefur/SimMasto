...
for(i in 1:3)
{
  ...
  tac = eval(parse(text=paste("random_TAC_152params_Y",i,sep="")))
    ...
    print(names(Iprinc_ssr)[Iprinc_ssr>0.01])
    ...
    print(names(Iprinc_amp)[Iprinc_amp>0.01])
    ...
    print(names(Iprinc_tac)[Iprinc_tac>0.01])
    
    jpeg(filename = paste("G:/DossiersASynchroniser/AnalysesSensibilite/AtelierAFH/Graphiques/PIQ3_aovIndices",i,".jpeg",sep=""), width = 900)
    par(mfrow = c(1,3))
